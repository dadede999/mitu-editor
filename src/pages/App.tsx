...
import { saveTpl } from '@/config/api/tpl';
import {
  ArrowLeftOutlined,
  FontSizeOutlined,
  PictureOutlined,
...
export default function App(props) {
  ...
  const updateAttr = (
    type: 'fill' | 'stroke' | 'strokeWidth' | 'imgUrl' | 'fontFamily',
    val: string | number,
  ) => {
  ...
  useEffect(() => {
    ...
    canvasRef.current.on('mouse:down', function (options: any) {
      ...
      //   options.target,
      // );
      currentNode.current = options.target;
      if (options.target) {
        const { fill = '#0066cc', stroke, strokeWidth = 0 } = options.target;
  ...
  function handleFontChange(value: string) {
    ...
    myfont
      .load()
      .then(function () {
        canvasRef.current?.getActiveObject()?.set('fontFamily', value);
      ...
      <main className={styles.contentWrap}>
        ...
        <section className={styles.canvasWrap}>
          <div className={styles.controlWrap}>
            ...
            <div className={styles.rightArea}>
              ...
              <Button className={styles.btn} size="small" onClick={clear}>
                清空
              </Button>
              <Button
        ...
        <section className={styles.panelWrap}>
          ...
          <div className={styles.attrPanel}>
            ...
            />
            <span className={styles.label}>描边: </span>
            <input
              type="color"
              value={attrs.stroke}
              style={{ width: 60 }}
            ...
            <InputNumber
              ...
              style={{ width: 60 }}
              onChange={(v) => updateAttr('strokeWidth', v)}
            />
            {currentNode.current?.text && (
              <Font handleFontChange={handleFontChange} />
            )}
          </div>
          ...
          <div className={styles.shapeWrap}>
            ...
            <div className={styles.shape}>
              <Tooltip placement="bottom" title="点击使用">
                <div
                  className={styles.tringle}
                  onClick={() => insertElement('Triangle')}
                ></div>
              </Tooltip>
            </div>
            <div className={styles.shape}>
          ...
          <div className={styles.operationArea}>
            ...
            <Button block className={styles.control} onClick={showTplModal}>
              保存为模版
            </Button>
          </div>
        </section>
